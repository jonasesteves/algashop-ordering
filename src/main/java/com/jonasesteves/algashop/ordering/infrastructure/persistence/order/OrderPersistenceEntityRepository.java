...
public interface OrderPersistenceEntityRepository extends JpaRepository<OrderPersistenceEntity, Long> {
    ...
    """)
    List<OrderPersistenceEntity> placedByCustomerInYear(@Param("customerId") UUID customerId, @Param("year") Integer start);
    ...
    @Override
    @EntityGraph(attributePaths = {"customer", "items"})  //Informa ao JPA para trazer também estes atributos.
