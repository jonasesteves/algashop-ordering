package com.jonasesteves.algashop.ordering.application.order.query;
...
class OrderQueryServiceIT {
    ...
    void shouldFindById() {
        ...
        Order order = OrderTestDataBuilder.someOrder().customerId(customer.id()).build();
        orders.add(order);
    ...
    void shouldFilterByPage() {
        ...
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.DRAFT).withItems(false).customerId(customer.id()).build());
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.PLACED).customerId(customer.id()).build());
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.PAID).customerId(customer.id()).build());
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.READY).customerId(customer.id()).build());
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.CANCELED).customerId(customer.id()).build());
    ...
    void shouldFilterByMultipleParams() {
        ...
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.DRAFT).withItems(false).customerId(customer1.id()).build());
        Order order1 = OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.PLACED).customerId(customer1.id()).build();
        orders.add(order1);

        Customer customer2 = CustomerTestDataBuilder.existingCustomer().id(new CustomerId()).build();
        customers.add(customer2);
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.PAID).customerId(customer2.id()).build());
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.READY).customerId(customer2.id()).build());
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.CANCELED).customerId(customer2.id()).build());
    ...
    void shouldOrderByStatus() {
        ...
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.DRAFT).withItems(false).customerId(customer1.id()).build());
        orders.add(OrderTestDataBuilder.someOrder().orderStatus(OrderStatus.PLACED).customerId(customer1.id()).build());
